...
export default function FeesPage() {
  ...
  if (userRole === 19) {
    const feesData = { totalFees: 50000, paidAmount: 35000, pendingAmount: 15000, dueDate: '2024-02-28' }
    const feeHistory = [
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          {[
            { label: 'Total Fees', value: feesData.totalFees, color: 'text-gray-900' },
            { label: 'Paid Amount', value: feesData.paidAmount, color: 'text-green-600' },
            { label: 'Pending Amount', value: feesData.pendingAmount, color: 'text-red-600' },
          ].map((item) => (
            <Card key={item.label}>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
              <div className="bg-green-600 h-4 rounded-full transition-all" style={{ width: `${(feesData.paidAmount / feesData.totalFees) * 100}%` }} />
            </div>
            {feesData.pendingAmount > 0 && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-sm text-yellow-800"><span className="font-semibold">Due Date:</span> {feesData.dueDate}</p>
                <p className="text-sm text-yellow-800 mt-1">Please pay the pending amount of ₹{feesData.pendingAmount.toLocaleString()} before the due date.</p>
              </div>
            )}
          </CardContent>
        </Card>
        <Card>
          <CardHeader><h3 className="text-xl font-semibold text-gray-900">Payment History</h3></CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {feeHistory.map((rec, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50">
                      <td className="py-3 px-4 text-sm text-gray-900">{rec.date}</td>
                      <td className="py-3 px-4 text-sm text-gray-900">{rec.description}</td>
                      <td className="py-3 px-4 text-sm text-gray-900 text-right">₹{rec.amount.toLocaleString()}</td>
                      <td className="py-3 px-4">
                        <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${rec.status === 'Paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{rec.status}</span>
                      </td>
                      <td className="py-3 px-4 text-sm text-gray-900">{rec.receipt}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
